...
const ray3Anim = gsap.to("#ray3", {rotate: 360, duration: 110, scaleX: 1.2, direction: "left", repeat: 11, transformOrigin: "center", yoyo: true, ease: "elastic.out(1, 0.3)"}).pause();
const pupilAnim = gsap.to("#pupil", { scaleX: (Math.random() * (1.2 - .9) + .9), scaleY: (Math.random() * (1.2 - .9) + .9), transformOrigin: "center", yoyo: true, repeat: 11}).pause();
...
container.addEventListener('mousemove', e => {
    ...
    gsap.to("#iris-shadow", {x: intersection[0], y: intersection[1] > 0 ? intersection[1] * .7 : intersection[1] , ease: "slow(0.7, 0.7, false)"});
    ...
    gsap.to("#eye-shadow", { x: intersection[0] * .12})
